...
sim_null_4GM_select_sd <- function(P, N_1, N_2, Pi_11, Pi_10, Pi_01, Pi_00, beta, rho, noise, threshold, q_u, b_u){
  ...
  # Exposure data
  y0 <- X_1%*%W[, 1]
  y <- y0 + noise*sqrt((1-q_u)*drop(var(y0)))*rnorm(N_1)
  # Oucome data
  z0 <- X_2%*%W[, 2]
  # z <- z0 + noise*sqrt((1-q_u)*drop(var(z0)))*rnorm(N_2)
  # Exposure data (MR)
  y03 <- X_3%*%W[, 1]
  y3 <- y03 + noise*sqrt((1-q_u)*drop(var(y03)))*rnorm(N_1)
  
  # confounder U
  u <- noise*sqrt(q_u*drop((var(y0)+var(y03))/2))*rnorm(N_1)
  y <- y + u
  y3 <- y3 + u
  z <- z0 + b_u*u + noise*sqrt(drop(var(z0)) - drop(var(b_u*u)))*rnorm(N_1) 
...
for (i in 1:Rp) {
  ...
  res <- sim_null_4GM_select_sd(P = 10000, N_1 = 5000, N_2 = 5000,
                                Pi_11 = 0.08, Pi_10 = 0.02, Pi_01 = 0.02, Pi_00 = 0.88,
                                beta = 0.0, rho = 0, noise = sqrt(1/1), threshold = c(1e-5),
                                q_u = 0.1, b_u = 0.5)
  
  for (k in 1:1){ # k thresholds
    ## unbias
    dat <- res$unbias[[k]]
    N <- dim(dat)[1]
    message("N=", N)
...
save.image("pl8-seed1-gpu-U-qu01-bu05.RData")
