##### Case-3 in summary-level simulations (qq-plot, type I error rate, power) ####
# Figures S13 in Supplementary Document
...
sim_sqtau_corrupt <- function(beta_true, sqtau_true, sqsigma_true, N, C, sqtau_corrupted, sigmaX_true, sigmaY_true){
  ...
  Gammahat <- rnorm(N*(1-C), beta_true*gamma_true[1:(N*(1-C))], sqrt((sigmaY_true^2 + sqtau_true)[1:(N*(1-C))]))
  Gammahat <- c(Gammahat, rnorm(N*C, beta_true*gamma_true[(N*(1-C)+1):N], sqrt((sigmaY_true^2 + sqtau_corrupted)[(N*(1-C)+1):N])))
...
save.image("case-3-typeIerror-power.RData")
...
library(ggplot2)
library(reshape2)
load("case-3-typeIerror-power.RData")
