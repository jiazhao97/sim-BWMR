##### Case-2 in summary-level simulations (qq-plot, type I error rate, power) ####
...
sim_beta_corrupt <- function(beta_true, sqtau_true, sqsigma_true, N, C, beta_corrupted, sigmaX_true, sigmaY_true){
  ...
  Gammahat <- c(Gammahat, rnorm(N*C, beta_corrupted*gamma_true[(N*(1-C)+1):N], sqrt((sigmaY_true^2 + sqtau_true)[(N*(1-C)+1):N])))
...
save.image("case-2-typeIerror-power.RData")
...
library(ggplot2)
library(reshape2)
load("case-2-typeIerror-power.RData")
