...
save.image("weights-N20-C01-cortau3.RData")
...
a <- ls()
rm(list=a[which(a!='w_mat' & a !='b_set')])
save.image("weights-N20-C01-cortau5.RData")
...
a <- ls()
rm(list=a[which(a!='w_mat' & a !='b_set')])
save.image("weights-N20-C01-cortau1.RData")
...
plt <- ggplot(df_plot, aes(x=indx, y=value, fill=Statistics)) +
  ...
  labs(x = "Observation No.", title = "tau_corrupted=1") +
...
plt <- ggplot(df_plot, aes(x=variable, y=value)) +
  ...
  labs(x = "tau_corrupted", title = "beta estimates") +
  theme(axis.title.y = element_blank()) +
  theme(axis.title.y = element_blank()) +
  theme(legend.position = "top") +
  theme(axis.title.x = element_text(size = 20, vjust = -1.5),
