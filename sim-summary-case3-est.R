##### Case-3 in summary-level simulations (estimates) #####
# Figures S10, S11 and S12 in Supplementary Document
...
sim_sqtau_corrupt <- function(beta_true, sqtau_true, sqsigma_true, N, C, sqtau_corrupted, sigmaX_true, sigmaY_true){
  gamma_true <- rnorm(N, 0, sqrt(sqsigma_true))
  gammahat <- rnorm(N, gamma_true, sigmaX_true)
  
  # the first N*(1-C) data points of Gammahat are reliable
  # the last N*C data points of Gammahat are corrupted
  h <- as.integer(N*(1-C))
  Gammahat <- rnorm(h, beta_true*gamma_true[1:h], sqrt((sigmaY_true^2 + sqtau_true)[1:h]))
  Gammahat <- c(Gammahat, rnorm((N-h), beta_true*gamma_true[(h+1):N], sqrt((sigmaY_true^2 + sqtau_corrupted)[(h+1):N])))
...
for (k in 1:L) {
  ...
  for (j in 1:L1) {
    ...
    for (iter in 1:Rp) {
      ...
      dat <- sim_sqtau_corrupt(beta_true, sqtau_true, sqsigma_true, N, C, sqtau_corrupted, sigmaX_true, sigmaY_true)
...
sqsigma_true <- 0.8^2
# declare sets
b.bwmr.C <- matrix(nrow = L, ncol = Rp*L3)
b.raps.C <- matrix(nrow = L, ncol = Rp*L3)
b.gsmr.C <- matrix(nrow = L, ncol = Rp*L3)
b.egger.C <- matrix(nrow = L, ncol = Rp*L3)
for (k in 1:L) {
  beta_true <- beta_choice[k]
  i <- 0
  for (j in 1:L3) {
    ...
    for (iter in 1:Rp) {
      ...
      res_gsmr = try(gsmr(bzx = dat$b.exposure, bzx_se = dat$se.exposure,  
                          ...
                          heidi_outlier_flag = T, 
                          nsnps_thresh = 1))
      b.gsmr.C[k, i] <- drop(res_gsmr$bxy)
      ## Two sample MR
      dat$id.exposure <- rep("uVXQCX", nrow(dat))
      dat$id.outcome <- rep("uVXQCX", nrow(dat))
      dat$mr_keep <- rep(TRUE, nrow(dat))
      ...
      colnames(dat) = c("beta.exposure", "beta.outcome", "se.exposure", "se.outcome", "id.exposure", "id.outcome", "mr_keep", "exposure", "outcome")
      result <- try(mr(dat, method_list=c("mr_raps", "mr_egger_regression")))
      b.raps.C[k, i] <- result$b[1]
      b.egger.C[k, i] <- result$b[2]
    }
  }
}
...
a <- ls()
rm(list=a[which(a!='b.bwmr.tau' & a !='b.egger.tau' & a !='b.gsmr.tau' & a !='b.raps.tau' & a!='b.bwmr.sigma' & a !='b.egger.sigma' & a !='b.gsmr.sigma' & a !='b.raps.sigma' & a!='b.bwmr.C' & a !='b.egger.C' & a !='b.gsmr.C' & a !='b.raps.C')])
save.image("case-3-estimate.RData")
...
df_boxplot_est <- data.frame(
  beta.num = rep(beta_num, rep(150, 3)),
  beta.title = rep(beta_0, rep(150, 3)),
  C.num = factor(rep(rep(C_num, rep(50, 3)), 3)),
  BWMR = c(b.bwmr.C[1, ], b.bwmr.C[2, ], b.bwmr.C[3, ]),
  RAPS = c(b.raps.C[1, ], b.raps.C[2, ], b.raps.C[3, ]),
  GSMR = c(b.gsmr.C[1, ], b.gsmr.C[2, ], b.gsmr.C[3, ]),
  Egger = c(b.egger.C[1, ], b.egger.C[2, ], b.egger.C[3, ])
)
df_boxplot_est <- melt(df_boxplot_est, id.vars = c("beta.num", "beta.title", "C.num"),
                       variable.name = 'Method', value.name = 'beta.est')
df_boxplot_est$Method <- factor(df_boxplot_est$Method, levels = method_level)
# plt
plt_boxplot_est <- ggplot(df_boxplot_est, aes(x = C.num, y = beta.est)) + 
  geom_boxplot(aes(fill = Method)) + 
  geom_hline(aes(yintercept = beta.num), colour = "blue", linetype = "dashed", size = 1) +
  labs(x = "True value of corrupted data rate C", title = "Estimation") +
  theme(axis.title.y = element_blank()) +
  theme(legend.position = "top") +
  theme(axis.title.x = element_text(size = 20), 
        plot.title = element_text(hjust = 0.5, size=25),
