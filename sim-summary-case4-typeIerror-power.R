##### Case-4 in summary-level simulations (qq-plot, type I error rate, power) ####
# Figures S16 in Supplementary Document
...
sim_without_outlier_mixn <- function(beta_true, sqtau_true, sqsigma_true, N, C, sigmaX_true, sigmaY_true){
  h <- as.integer(N*(1-C))
  gamma_true <- c(rnorm(h, 0, sqrt(sqsigma_true)), rnorm((N-h), 0, sqrt(10*sqsigma_true)))
...
for (k in 1:L) {
  ...
  for (iter in 1:Rp) {
    ...
    dat <- sim_without_outlier_mixn(beta_true, sqtau_true, sqsigma_true, N, C, sigmaX_true, sigmaY_true)
...
save.image("case-4-typeIerror-power.RData")
...
library(ggplot2)
library(reshape2)
load("case-4-typeIerror-power.RData")
